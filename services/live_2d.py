...
def main():
    ...
    if live2d.LIVE2D_VERSION == 3:
        model.LoadModelJson(
            # os.path.join(RESOURCES_DIRECTORY, "jinglu/jingliu/jingliu.model3.json"),
            os.path.join(RESOURCES_DIRECTORY, "Sparkle/Sparkle.model3.json"),
            maskBufferCount=100,
        )
    else:
        model.LoadModelJson(
            # os.path.join(RESOURCES_DIRECTORY, "v2/托尔/model0.json")
            os.path.join(RESOURCES_DIRECTORY, "PB3/PB3.model3.json")
        )
    ...
    def on_start_motion_callback(group: str, no: int):
        ...
        audioPath = os.path.join(CURRENT_DIRECTORY, "lazlo.wav")
        print("fuck", audioPath)
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                model.StartRandomMotion(
                    priority=3, onFinishMotionHandler=on_finish_motion_callback
                )
