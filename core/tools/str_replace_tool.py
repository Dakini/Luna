...
from core.types.agent_types import ToolCallParameters, ToolImplOutput
...
from core.utils.indent_utils import match_indent, match_indent_by_first_line
...
"""
        ...
        try:
            ...
            if not is_path_in_directory(self.workspace_manager.root, _ws_path):
                return ExtendedToolImplOutput(
                    tool_output=f"Path {_ws_path} is outside the workspace root directory: {container_root}. You can only access files within the workspace root directory.",
                    tool_result_message=f"Path {_ws_path} is outside the workspace root directory: {container_root}. You can only access files within the workspace root directory.",
            ...
            if command == "view":
                ...
                return ExtendedToolImplOutput(
                    tool_output=f"File created successfully at: {self.workspace_manager.container_path(path)}",
                    tool_result_message=f"File created successfully at: {self.workspace_manager.container_path(path)}",
                ...
                if self.ignore_indentation_for_str_replace:
                    ...
                    try:
                        ...
                        return ExtendedToolImplOutput(
                            tool_output=f"The file {path} could not be edited due to lack of permission. Try changing the file permissions.",
                            tool_result_message=f"The file {path} could not be edited due to lack of permission. Try changing the file permissions.",
                            auxiliary_data={"success": True},
                        )
            elif command == "insert":
            ...
            return ExtendedToolImplOutput(
                tool_output=e.message,  # pyright: ignore[reportAttributeAccessIssue]
                tool_result_message=e.message,  # pyright: ignore[reportAttributeAccessIssue]
                auxiliary_data={"success": False},
            )
    ...
    def view(
        ...
        if path.is_dir():
            ...
            return ExtendedToolImplOutput(
                tool_output=output,
                tool_result_message="Listed directory contents",
                auxiliary_data={"success": not stderr},
            )
        ...
        return ExtendedToolImplOutput(
            tool_output=output,
            tool_result_message="Displayed file content",
            auxiliary_data={"success": True},
        )
    ...
    def _str_replace_ignore_indent(self, path: Path, old_str: str, new_str: str | None):
        ...
        return self.create_extended_tool_impl(
            tool_output=success_msg,
            tool_result_message=f"The file {path} has been edited.",
            auxiliary_data={"success": True},
        )
    ...
    def create_extended_tool_impl(
        self, tool_output, tool_result_message, auxiliary_data=None
    ):
    ...
    def insert(
        ...
        return ExtendedToolImplOutput(
            tool_output=success_msg,
            tool_result_message="Insert successful",
            auxiliary_data={"success": True},
        )
    ...
    def undo_edit(self, path: Path) -> ExtendedToolImplOutput:
        ...
        return ExtendedToolImplOutput(
            tool_output=output,
            tool_result_message="Undo successful",
            auxiliary_data={"success": True},
        )
