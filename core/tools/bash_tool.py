    ...
    def run_impl(
        ...
        for banned_str in self.banned_command_strs:
            if banned_str in command:
                return ToolImplOutput(
                    tool_output=f"Command not executed due to banned string in command: {banned_str} found in {command}.",
                    tool_result_message=f"Command not executed due to banned string in command: {banned_str} found in {command}.",
                    auxiliary_data=aux_data
                    | {"success": False, "reason": "Banned command"},
                )
        ...
        if self.require_confirmation:
            ...
            if confirmation.lower() != "y":
                return ToolImplOutput(
                    tool_output="Command not executed due to lack of user confirmation.",
                    tool_result_message="Command execution cancelled",
                    auxiliary_data=aux_data
                    | {"success": False, "reason": "User did not confirm"},
                )
        ...
        try:
            ...
            if "Timeout exceeded." in str(e):
                return ToolImplOutput(
                    tool_output="Command timed out. Please try again.",
                    tool_result_message="Command timed out. Please try again.",
                    auxiliary_data=aux_data | {"success": False},
                )
            return ToolImplOutput(
                tool_output=f"Error executing command: {str(e)}",
                tool_result_message=f"Failed to execute command '{original_command}'",
                auxiliary_data=aux_data
                | {
                    "success": False,
                    "error": str(e),
        ...
        return ToolImplOutput(
            tool_output=result,
            tool_result_message=f"Command '{command}' executed.",
            auxiliary_data=aux_data | {"success": True},
        )
